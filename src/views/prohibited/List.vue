					...
					<el-col :span="12">
				        <el-form-item label="海报" prop="img" >
							...
							<div style="text-align:center;margin-left:25px;float:left;margin-top:18px;">
								<el-button size="mini" type="primary" plain  @click="mmm" v-permission="['movie:update']">
								上传海报
								</el-button>
							</div>
				        </el-form-item>
				...
				<el-row>
				    <el-col :span="24">
				        <el-form-item label="剧情简介" prop="synopsis" style="margin-top:-15px">
				            <el-input v-model="form.synopsis" type="textarea" placeholder="请输入剧情简介" maxlength="30" :rows="5"/>
				        </el-form-item>
				    </el-col>
				</el-row>
            ...
            return{
                ...
                rules: {
					...
					year:[
						...
						{ min:4,max: 4, message: '年份必须为4位', trigger: 'blur' }
					],
					duration:[
						//ransform：向验证规则添加一个函数，该属性在验证之前进行转换，并重新分配给源对象，以便在适当的位置改变属性的类型
						{ type: 'string',required: true, message: "时长不能为空", trigger: "blur", transform: (value) => String(value)},
						{ max: 3, message: '昵称长度必须介于 1 和 3之间', trigger: 'blur' }
					],
					area:[
        ...
        methods: {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid){
                        if (this.title === "添加电影"){
							...
							this.form.img="https://img2.doubanio.com/view/photo/s_ratio_poster/public/p2545472803.webp"
            ...
            handleDelete(row){
                const movieIds = row.id || this.ids
                    this.modal.confirm('确定要删除电影吗？').then(function () {
                    }).then(()=>{
